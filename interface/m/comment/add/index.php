...
if (!doesHaveOwnership() && empty($_POST["name_$entryId"])) {
	printMobileErrorPage(_t('댓글을 작성할 수 없습니다.'), _t('이름을 입력해 주십시오.'), "$blogURL/comment/$entryId");
} else if (!doesHaveOwnership() && empty($_POST["comment_$entryId"])) {
	printMobileErrorPage(_t('댓글을 작성할 수 없습니다.'), _t('본문을 입력해 주십시오.'), "$blogURL/comment/$entryId");
} else {
	$comment = array();
