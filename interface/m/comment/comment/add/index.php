...
if (!doesHaveOwnership() && empty($_POST["name_$replyId"])) {
	printMobileErrorPage(_t('댓글을 작성할 수 없습니다.'), _t('이름을 입력해 주십시오.'), "$blogURL/comment/comment/$replyId");
} else if (!doesHaveOwnership() && empty($_POST["comment_$replyId"])) {
	printMobileErrorPage(_t('댓글을 작성할 수 없습니다.'), _t('본문을 입력해 주십시오.'), "$blogURL/comment/comment/$replyId");
} else {
	$comment = array();
	...
	if ($result === 'blocked') {
		printMobileErrorPage(_t('댓글쓰기가 차단됐습니다.'), "$blogURL/comment/$entryId");
	} else if ($result === false) {
		printMobileErrorPage(_t('댓글을 쓸 수 없습니다.'), "$blogURL/comment/$entryId");
	} else {
		setcookie('guestName', $comment['name'], time() + 2592000, $blogURL);
		setcookie('guestHomepage', $comment['homepage'], time() + 2592000, $blogURL);
		printMobileSimpleMessage(_t('댓글이 작성됐습니다.'), _t('댓글 보기 화면으로'), "$blogURL/comment/$entryId");
	}
}
?>
