...
if(!file_exists(ROOT . '/cache/CHECKUP')) $currentVersion = _t('첫번째 점검');
...
function clearCache() {
	...
	if(!is_null($blogids = POD::queryColumn("SELECT blogid FROM {$database['prefix']}PageCacheLog"))) {
		$changed = true;
		$errorlog = false;
		echo '<li>', _f('페이지 캐시를 초기화합니다.'), ': ';
		foreach($blogids as $ids) {
			if(CacheControl::flushAll($ids) == false) $errorlog = true; 
		}
		if($errorlog == false) echo '<span class="result success">', _t('성공'), '</span></li>';
		else echo '<span class="result fail">', _t('실패'), '</span></li>';
	}
	if(!is_null($memcache)) {
		echo '<li>', _f('Memcached 캐시를 초기화합니다.'), ': ';
		if($memcache->flush())  echo '<span class="result success">', _t('성공'), '</span></li>';
		else echo '<span class="result fail">', _t('실패'), '</span></li>';
	}
	echo '<li>', _f('공지사항 캐시를 초기화합니다.'), ': ';
...
<head>
	...
	<title><?php echo _t('텍스트큐브를 점검합니다.');?></title>
...
<body>
	<div id="container">
		<form id="checkup">
			...
			<div id="inner">
				<h2><?php echo _t('텍스트큐브 점검을 시작합니다.');?></h2>
				
				<div id="content">
					<h3><?php echo _t('버전 검사');?></h3>
					
					<ul class="version">
						<li><?php echo _f('기존 버전 - %1',$currentVersion);?></li>
						<li><?php echo _f('현재 버전 - %1',TEXTCUBE_VERSION);?></li>
					</ul>
<?php
	if(version_compare($currentVersion,'1.8.0') < 0) {
?>
					<h3><?php echo _t('업그레이드 안내');?></h3>
					<ul id="upgradeInstruction">
						<li class="instruction"><?php echo _t('텍스트큐브 1.8 미만에서 텍스트큐브로 이전할 경우, 기본 스킨의 저장 위치가 아래와 같이 변경됩니다.').' '._t('기존의 스킨들을 새 디렉토리로 옮겨 주시기 바랍니다.');?></li>
						<li class="detail"><?php echo _f('스킨 디렉토리 : %1 에서 %2로 변경','/skin/','/skin/blog/');?></li>
						<li class="detail"><?php echo _f('관리 패널 스킨 디렉토리 : %1 에서 %2로 변경','/style/admin/','/skin/admin/');?></li>
					</ul>

<?php
	}
?>
					<h3><?php echo _t('변경 중');?></h3>
...
if($currentVersion != TEXTCUBE_VERSION && in_array(POD::dbms(),array('MySQL','MySQLi'))) {
	...
	if (POD::queryCell("DESC {$database['prefix']}CommentsNotified id", 'Extra') == 'auto_increment') {
		$changed = true;
		echo '<li>', _t('데이터베이스 호환성을 위하여 댓글 테이블의 자동 증가 설정을 제거합니다.'), ': ';
	...
	if (POD::queryCell("DESC {$database['prefix']}Trackbacks id", 'Extra') == 'auto_increment') {
		$changed = true;
		echo '<li>', _t('데이터베이스 호환성을 위하여 트랙백 테이블의 자동 증가 설정을 제거합니다.'), ': ';
	...
	if (POD::queryCell("DESC {$database['prefix']}Comments blogid", 'Key') != 'PRI') {
		$changed = true;
		echo '<li>', _t('데이터베이스 호환성을 위하여 댓글 테이블의 인덱스 설정을 변경합니다.'), ': ';
	...
	if (!doesExistTable($database['prefix'] . 'EntriesArchive')) {
		$changed = true;
		echo '<li>', _t('글 버전 관리및 비교를 위한 테이블을 추가합니다.'), ': ';
		$query = "
	...
	if (!doesExistTable($database['prefix'] . 'OpenIDUsers')) {
		$changed = true;
		echo '<li>', _t('오픈아이디 사용자 테이블을 만듭니다'), ': ';
		$query = "
	...
	if (!POD::queryExistence("DESC {$database['prefix']}Comments openid")) {
		$changed = true;
		echo '<li>', _t('Comments 테이블에 openid 필드를 추가합니다.'), ': ';
	...
	if (doesExistTable($database['prefix'] . 'OpenIDComments')) {
		$changed = true;
		echo '<li>', _t('오픈아이디 댓글 테이블을 기존 댓글 테이블에 병합합니다'), ': ';
	...
	if (POD::queryExistence("DESC {$database['prefix']}Links visible")) {
		$changed = true;
		echo '<li>', _t('Links 테이블의 공개 여부 설정 필드의 속성을 변경합니다.'), ': ';
	...
	if (!POD::queryExistence("DESC {$database['prefix']}Links visibility")) {
		$changed = true;
		echo '<li>', _t('Links 테이블에 공개 여부 설정 필드와 XFN 마이크로포맷을 위한 필드를 추가합니다.'), ': ';
	...
	if (POD::queryCell("DESC {$database['prefix']}Sessions updated", 'Key') != 'MUL') {
		$changed = true;
		echo '<li>', _t('동시 접속자 관리를 위하여 세션 테이블의 인덱스 설정을 변경합니다.'), ': ';
	...
	if (!POD::queryExistence("DESC {$database['prefix']}RemoteResponses blogid") &&
		...
		$changed = true;
		echo '<li>', _t('트랙백 불러오기 속도를 개선하기 위하여 트랙백 테이블의 인덱스 설정을 변경합니다.'), ': ';
	...
	if (POD::queryExistence("DESC {$database['prefix']}SessionVisits blog")) {
		$changed = true;
		echo '<li>', _t('SessionVisits 테이블의 블로그 정보 필드 이름을 변경합니다.'), ': ';
	...
	if (POD::queryCell("DESC {$database['prefix']}BlogSettings name", 'Key') != 'PRI') {
		$changed = true;
		echo '<li>', _t('블로그 설정 불러오기 속도를 개선하기 위하여 블로그 설정 테이블의 인덱스 설정을 변경합니다.'), ': ';
	...
	!POD::queryExistence("DESC {$database['prefix']}SkinSettings showListOnAuthor")) {
		$changed = true;
		echo '<li>', _t('스킨 설정 테이블에 저자별 페이지 출력 설정을 위한 필드를 추가합니다.'), ': ';
	...
	if (POD::queryCell("DESC {$database['prefix']}Entries draft", 'Key') != 'PRI') {
		$changed = true;
		echo '<li>', _t('엔트리 테이블의 주 인덱스에 draft를 추가합니다.'), ': ';
	...
	if (!POD::queryExistence("DESC {$database['prefix']}Entries starred")) {
		$changed = true;
		echo '<li>', _t('본문 테이블에 별표 및 작성 중 글 표시를 위한 필드를 추가합니다.'), ': ';
	...
	if (POD::queryCell("DESC {$database['prefix']}Users name", 'Key') != 'UNI') {
		$changed = true;
		echo '<li>', _t('id의 도용을 막기 위하여 같은 사용자 id를 사용할 수 없도록 합니다.'), ': ';
	...
	if (!is_null($notices = POD::queryAll("SELECT blogid, id, title, slogan
		...
		$changed = true;
		echo '<li>', _t('fancyURL이 적용되지 않는 공지 글에 슬로건을 추가합니다.'), ': ';
	...
	if (!POD::queryExistence("DESC {$database['prefix']}Filters filtertype") &&
		...
		$changed = true;
		echo '<li>', _t('필터 테이블에 예외 목록을 추가하기 위하여 필드 속성을 변경합니다.'), ': ';
	...
	if (!POD::queryExistence("DESC {$database['prefix']}RemoteResponses blogid")) {
		$changed = true;
		echo '<li>', _t('원격 댓글 지원 기능을 위해 트랙백 테이블의 이름을 변경합니다.'), ': ';
		if (
	...
	if(!POD::queryExistence("DESC {$database['prefix']}RemoteResponses responsetype")) {
		$changed = true;
		echo '<li>', _t('트랙백 테이블에 컨텐츠 종류를 판단하기 위한 필드를 추가합니다.'), ': ';
	...
	if (!POD::queryExistence("DESC {$database['prefix']}Entries pingbacks")) {
		$changed = true;
		echo '<li>', _t('핑백 기능을 위해 글 테이블에 핑백 필드를 추가합니다.'), ': ';
	...
	if (!POD::queryExistence("DESC {$database['prefix']}Privileges acl")) {
		$changed = true;
		echo '<li>', _t('권한 관리 테이블의 이름을 변경합니다.'), ': ';
	...
	if (!DBAdapter::queryExistence("DESC {$database['prefix']}Entries longitude")) {
		$changed = true;
		echo '<li>', _t('글과 위경도 좌표 연동을 위한 필드를 추가합니다.'), ': ';
	...
	if (!DBAdapter::queryExistence("DESC {$database['prefix']}Comments longitude")) {
		$changed = true;
		echo '<li>', _t('댓글과 위경도 좌표 연동을 위한 필드를 추가합니다.'), ': ';
	...
	if (!POD::queryExistence("DESC {$database['prefix']}RemoteResponses responsetype")) { 
		$changed = true;
		echo '<li>', _t('트랙백 테이블의 호환성을 위하여 필드 이름을 변경합니다.'), ': ';
	...
	if (POD::queryCell("DESC {$database['prefix']}Attachments name", 'Type') == 'varchar(32)') {
		$changed = true;
		echo '<li>', _t('티스토리 데이터 백업 호환성을 위하여 첨부파일 이름 필드 크기를 확장합니다.'), ': ';
	...
	if (POD::queryCell("DESC {$database['prefix']}FeedItems id", 'Extra') == 'auto_increment') {
		$changed = true;
		echo '<li>', _t('다양한 데이터베이스 엔진 호환성을 위하여 자동 증가 설정을 제거합니다.'), ': ';
		if (
	...
	if (POD::queryExistence("DESC {$database['prefix']}SkinSettings recentCommentLength")) {
		$changed = true;
		echo '<li>', _t('스킨 설정 테이블의 구조를 변경합니다.'), ': ';
		if(POD::queryExistence("DESC {$database['prefix']}SkinSettings recentCommentLength")) {
			$lowerChar = true;	
	...
	if (!POD::queryExistence("DESC {$database['prefix']}DailyStatistics datemark")) {
		$changed = true;
		echo '<li>', _t('다양한 데이터베이스 엔진 호환성을 위하여 모든 필드의 이름을 소문자로 변환합니다.'), ': ';
		if (
	...
	if (!POD::queryExistence("DESC {$database['prefix']}FeedSettings loadimage")) { 
		$changed = true;
		echo '<li>', _t('피드 설정 테이블의 테이블 구조 오류를 보정합니다.'), ': ';
	...
	if (!POD::queryExistence("DESC {$database['prefix']}OpenIDUsers loginCount")) { 
		$changed = true;
		echo '<li>', _t('OpenID 사용자 테이블의 테이블 구조 오류를 보정합니다.'), ': ';
	...
	if (!POD::queryExistence("DESC {$database['prefix']}Filters filtertype")) {
		$changed = true;
		echo '<li>', _t('필터 호환성을 위하여 필드의 이름을 변경합니다.'), ': ';
	...
	if (!doesExistTable($database['prefix'] . 'Lines')) {
		$changed = true;
		echo '<li>', _t('라인 기능을 위한 테이블을 만듭니다'), ': ';
		$query = "
	...
	if (!DBAdapter::queryExistence("DESC {$database['prefix']}Lines permalink")) {
		$changed = true;
		echo '<li>', _t('라인 기능에 여러 라인의 통합을 위한 필드를 추가합니다.'), ': ';
	...
	if (!doesExistTable($database['prefix'] . 'Widgets')) {
		$changed = true;
		echo '<li>', _t('위젯 기능 및 오픈소셜 지원을 위한 테이블을 만듭니다'), ': ';
		$query = "
...
?>
					...
					<p id="lastMessage">
						<?php
	reloadSkin(1);
	echo ($changed ? _t('완료되었습니다.') : _t('확인되었습니다.'));
?>
					</p>
				</div>
