...
if (isset($_GET['session']) && isset($_GET['requestURI'])) {
	...
	if (!$isLogin) {
		$message = _t('잘못된 E-mail 주소 또는 비밀번호입니다.');
...
if(!doesHaveOwnership()) {
	...
	<form id="Login" method="GET" action="<?php echo $blogURL;?>/login" title="Login" class="panel" selected="false">
        ...
        <fieldset>
			...
			<?php };?>
            <div class="row">
				<label for="loginid"><?php echo _t('E-mail');?></label>
            ...
            <div class="row">
				<label for="password"><?php echo _t('비밀번호');?></label>
            ...
            <div class="row">
                <label><?php echo _t('E-mail 저장');?></label>
		...
		<a href="#" class="whiteButton " type="submit"><?php echo _t('블로그 로그인');?></a>
	</form>
<?php
} else {
?>
	<div id="Login" title="Login" class="panel" selected="false">
		<div class="content">
			<?php echo _t('로그인 하였습니다.');?>
		</div>
		<a href="#" onclick="self.location.reload();" class="whiteButton margin-top10"><?php echo _t('첫 페이지로 돌아가기');?></a>
		<a href="<?php echo $defaultURL."/owner/center/dashboard";?>" onclick="window.location.href='<?php echo $defaultURL."/owner/center/dashboard";?>'" class="whiteButton margin-top10"><?php echo _t('관리 패널로 들어가기');?></a>
	</div>
<?php
}
?>
