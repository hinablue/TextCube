...
if (!doesHaveOwnership() && empty($_GET["name_$entryId"])) {
	printIphoneErrorPage(_t('Comment write error.'), _t('Please enter your name.'), "$blogURL/comment/$entryId");
} else if (!doesHaveOwnership() && empty($_GET["comment_$entryId"])) {
	printIphoneErrorPage(_t('Comment write error.'), _t('Please enter content.'), "$blogURL/comment/$entryId");
} else {
	$comment = array();
	...
	if (in_array($result, array('ip', 'name', 'homepage', 'comment', 'openidonly', 'etc'))) {
		if ($result == 'openidonly') {
			$blockMessage = _t('You have to log in with and OpenID to leave a comment.');
		} else {
			$blockMessage = _textf('Blocked %1', $result);
		}
		printIphoneErrorPage(_t('Comment write blocked.'), $blockMessage, "$blogURL/comment/$entryId");
	} else if ($result === false) {
		printIphoneErrorPage(_t('Comment write error.'), _t('Cannot write comment.'), "$blogURL/comment/$entryId");
	} else {
		setcookie('guestName', $comment['name'], time() + 2592000, $blogURL);
		setcookie('guestHomepage', $comment['homepage'], time() + 2592000, $blogURL);
		printIphoneSimpleMessage(_t('Comment registered.'), _t('Go to comments page'), "$blogURL/comment/$entryId");
	}
}
?>
