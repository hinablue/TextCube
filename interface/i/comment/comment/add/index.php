...
if (!doesHaveOwnership() && empty($_GET["name_$replyId"])) {
	printIphoneErrorPage(_t('댓글 작성 오류.'), _t('이름을 입력해 주세요.'), "$blogURL/comment/comment/$replyId");
} else if (!doesHaveOwnership() && empty($_GET["comment_$replyId"])) {
	printIphoneErrorPage(_t('댓글 작성 오류.'), _t('내용을 입력해 주세요.'), "$blogURL/comment/comment/$replyId");
} else {
	$comment = array();
	...
	if (in_array($result, array('ip', 'name', 'homepage', 'comment', 'openidonly', 'etc'))) {
		if ($result == 'openidonly') {
			$blockMessage = _t('댓글을 쓰기 위해서는 OpenID로 로그인해야 합니다.');
		} else {
			$blockMessage = _textf('%1 은 차단되었습니다.', $result);
		}
		printIphoneErrorPage(_t('댓글 작성이 차단되었습니다.'), $blockMessage, "$blogURL/comment/$entryId");
	} else if ($result === false) {
		printIphoneErrorPage(_t('댓글 작성 오류.'), _t('댓글을 작성할 수 없었습니다.'), "$blogURL/comment/$entryId");
	} else {
		setcookie('guestName', $comment['name'], time() + 2592000, $blogURL);
		setcookie('guestHomepage', $comment['homepage'], time() + 2592000, $blogURL);
		printIphoneSimpleMessage(_t('댓글이 등록되었습니다.'), _t('댓글 페이지로 이동'), "$blogURL/comment/$entryId");
	}
}
?>
