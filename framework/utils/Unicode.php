	...
	static function validate($str, $truncated = false) {
		...
		for ($i = 0; $i < $length; $i++) {
			...
			if ($high < 0x80) {
				...
				if (++$i >= $length)
					return $truncated;
				else if ((substr($str, $i, 1) & "\xC0") == "\x80")
					continue;
			} else if ($high < 0xF0) {
				if (++$i >= $length) {
					return $truncated;
			...
			switch (substr($str, $i, 1)) {
				...
				default:
					...
					if ($high < 0x20) { // Special Characters.
						...
						if (($i + 1 >= $strlen) || ((substr($str,$i + 1,1) & "\xC0") != "\x80"))
							$corrected .= $broken;
						else
							$corrected .= substr($str, $i, 1) . substr($str,$i + 1,1);
						$i += 1;
					} else if ($high < 0xF0) { // 3byte.
						if (($i + 2 >= $strlen) || ((substr($str,$i + 1,1) & "\xC0") != "\x80") || ((substr($str,$i + 2,1) & "\xC0") != "\x80"))
							$corrected .= $broken;
						else
							$corrected .= substr($str, $i, 1) . substr($str,$i + 1,1) . substr($str,$i + 2,1);
						$i += 2;
					} else if ($high < 0xF5) { // 4byte.
						if (($i + 3 >= $strlen) || ((substr($str,$i + 1,1) & "\xC0") != "\x80") || ((substr($str,$i + 2,1) & "\xC0") != "\x80") || ((substr($str,$i + 3,1) & "\xC0") != "\x80"))
							$corrected .= $broken;
						else
							$corrected .= substr($str, $i, 1) . substr($str,$i + 1,1) . substr($str,$i + 2,1) . substr($str,$i + 3,1);
						$i += 3;
