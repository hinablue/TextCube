...
$__text['IIS 6.0 이전 버전을 사용하시는 경우 Rewrite 모듈을 설치하려면, 오픈스소 무료 모듈을 제공하고 있는 <a href="http://www.codeplex.com/IIRF" target="_blank">Ionics Isapi Rewrite Filter 홈페이지</a>를 방문하여 설치하신 후, 계속 진행·설치 후 생성되는 <b>.htaccess</b> 파일의 내용을 위 모듈의 설정파일(<b>IsapiRewrite4.ini</b>)에 복사하시기 바랍니다.'] = '如果您使用IIS 6.0 等较早期的Rewrite 模组安装，我们建议您安装开放原始码的<a href="http://www.codeplex.com/IIRF" target="_blank">Ionics Isapi Rewrite Filter 模组</a>。并于安装此模组之后在返回安装程序。继续安装完成后，请复制所产生的<b>.htaccess</b> 内容，并写入该模组（<b>IsapiRewrite4.ini</b>）中。';
$__text['IIS 7.0용 공식 URL Rewrite 모듈을 사용합니다.'] = 'IIS 7.0 是官方预设选用的URL Rewrite 模组。';
$__text['IIS 7.0을 사용하시는 경우 공식 URL Rewrite Module을 사용하려면 <a href="http://www.iis.net/extensions/URLRewrite">이곳에서 다운로드</a>받아 설치하시고, 계속 진행·설치 후 생성되는 <b>.htaccess</b> 파일 내용을 그대로 import해주시면 됩니다.'] = '如果您要使用IIS 7.0 官方预设选用的URL Rewrite 模组，您可以<a href="http://www.iis.net/extensions/URLRewrite">在此下载</a> 并安装。继续安装完成后，请复制<b>.htaccess</b> 档案内容，并import 至模组中。';
...
$__text['데이터베이스 종류'] = '数据库类型';
$__text['데이터베이스 테이블 확인'] = '确认数据库表';
$__text['데이터베이스 포트'] = '数据库连接埠';
$__text['데이터베이스가 해당 호스트에 먼저 생성되어 있어야 합니다.'] = '使用的数据库需要先建立。';
...
$__text['만약 이 페이지를 보고 계시다면 Apache mod_rewrite와 호환되지 않는 Rewrite 모듈을 사용 중이거나 아예 모듈이 없는 경우입니다.'] = '如果您看到这个页面，表示您不使用Apache mod_rewrite 模组，或是关闭了Rewrite 模组的使用。';
$__text['버전'] = '版本';
...
$__text['새로 IIS용 Rewrite 모듈을 설치하셨다면 <b>.htaccess 내용을 모듈 설정에 적용</b>해주십시오.<br />'] = '若您使用IIS Rewrite 模组设置，请将<b>.htaccess 档案内容载入或更新</b>至该模组中。';
...
$__text['테이블'] = '数据表';
$__text['테이블식별자는 텍스트큐브가 사용하는 테이블이름 앞에 붙는 문자열입니다. 데이터 베이스내에 다른 어플리케이션이 사용하는 테이블이 있을 경우 구별하기 위해 사용합니다'] = '预设TextCube 使用的前缀字符，当TextCube 在其他资料库中使用时，可利用资料表前缀字符来区分不同的资料表。';
