...
function setStyleSave() {
	try {
		...
		request.onSuccess = function () {
			PM.showMessage("<?php echo _t('필명 스타일이 저장 되었습니다.');?>", "center", "bottom");
		}
		request.onError = function() {
			PM.showErrorMessage("<?php echo _t("저장하지 못했습니다. 다시 시도 해주세요.");?>", "center", "bottom");
		}
...
function setProfileSave() {
	try {
		...
		var request = new HTTPRequest("POST", blogURL + "/plugin/teamContentsSave/");
		request.onSuccess = function () {
			PM.showMessage("<?php echo _t("프로필 설명이 저장 되었습니다.");?>", "center", "bottom");
		}
		request.onError = function() {
			PM.showErrorMessage("<?php echo _t("저장하지 못했습니다. 다시 시도 해주세요.");?>", "center", "bottom");
		}
						...
						<dl id="team-image-line" class="line">
							...
							<dd>
								...
								<form id="file_upload_form" method="post" target="uploadTarget" enctype="multipart/form-data" action="<?php echo $blogURL;?>/plugin/teamFileUpload/">
									...
									<input type="file" name="teamImageFile" id="teamImageFile" size="35" onchange="uploadImage(this.form, 'upload');" /> <a href="http://mypictr.com/?size=<?php echo $data['imageSize']?>x<?php echo $data['imageSize']?>" onclick="window.open(this.href); return false;"><font color="#527A98"><u>mypictr.com</u></font></a>&nbsp;<?php echo _t('에서 사진 편집.');?><br />
