...
function queryLocation() {
	...
	geocoder.getLocations(q, function(response) {
		if (!response || response.Status.code != 200) {
			$('<div id="queryResult">'+_t['검색 결과가 없습니다.']+'</div>').insertAfter('#GoogleMapPreview');
		} else {
			...
			for (var i = 0; i < response.Placemark.length; i++) {
				...
				marker.bindInfoWindowHtml('<div class="queryMarkerInfo"><p><address>'+place.address+'</address></p><p><a href="#" onclick="convertToUserMarker(\''+id+'\')">'+_t['마커 고정시키기']+'</a></p></div>');
		...
		$('<div style="text-align:right"><a href="#" class="ui-action" onclick="closeQueryResult();return false;">'+_t['닫기']+'</a></div>').appendTo('#queryResult');
...
function GMarker_onClick(latlng) {
	...
	form += '<p><label for="info_title">'+_t['제목']+' : </label><input id="info_title" type="text" value="'+um.title+'" /></p>';
	form += '<p><label for="info_desc">'+_t['설명']+' : </label><textarea id="info_desc" rows="3" cols="30">'+um.desc+'</textarea></p>';
	form += '<div style="text-align:right"><a href="javascript:void(0);" onclick="removeUserMarker(\''+um.id+'\');">'+_t['삭제하기']+'</div></div>';
