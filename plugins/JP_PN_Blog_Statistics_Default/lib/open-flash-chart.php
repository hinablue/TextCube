...
class graph
{
    function graph()
    {
		...
		$this->bar_titles = array();
		$this->bar_colours = array();
    ...
    function set_bar_titles( $a)
    {
       $this->bar_titles = $a;//'&bar_titles='.implode(',',$a).'&'."\r\n";
    ...
    function pie_values( $values, $labels, $links, $titles)
    {
         $this->pie_values = implode(',',$values);
         $this->pie_labels = implode(',',$labels);
         $this->pie_titles = implode(',',$titles);
    ...
    function bar_colours( $colours )
    {
        $this->bar_colours = $colours;
    }
    ...
    function render()
    {
		...
		if( count( $this->bar_titles ) > 0 ){
                $tmp .= '&bar_titles='.implode(',',$this->bar_titles).'&'."\r\n";               
		}

		if( count( $this->bar_colours ) > 0 ){
                $tmp .= '&bar_colours='.implode(',',$this->bar_colours).'&'."\r\n";               
		}
        ...
        if( strlen( $this->pie ) > 0 )
        {
			 ...
			 $tmp .= '&pie_titles='. $this->pie_titles .'&'."\r\n";
