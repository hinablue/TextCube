							...
							//<![CDATA[
								function deleteComment(id) {
									if (!confirm("<?php echo _t('선택된 댓글 또는 방명록을 삭제합니다. 계속 하시겠습니까?');?>"))
										return;
									var request = new HTTPRequest("GET", "<?php echo $blogURL;?>/plugin/DEFENSIO/delete/?id=" + id + "&t=C");
								...
								function deleteCommentAll() {
									if (!confirm("<?php echo _t('모든 댓글 및 방명록을 삭제합니다. 계속 하시겠습니까?');?>"))
										return;
									var request = new HTTPRequest("GET", "<?php echo $blogURL;?>/plugin/DEFENSIO/quarantine/?t=C&ajaxcall");
									request.onSuccess = function () {
								...
								function deleteComments() {	
									...
									var request = new HTTPRequest("POST", "<?php echo $blogURL;?>/plugin/DEFENSIO/delete/?t=C");
									request.onSuccess = function() {
										document.getElementById('list-form').submit();
									}
									request.onError = function () {
								...
								function revertComment(id) {
									if (!confirm("<?php echo _t('선택된 댓글 및 방명록을 복원합니다. 계속 하시겠습니까?');?>"))
										return;
									var request = new HTTPRequest("GET", "<?php echo $blogURL;?>/plugin/DEFENSIO/revert/?id=" + id + "&t=C");
									request.onSuccess = function () {
										document.getElementById('list-form').submit();
									}
									request.onError = function () {
								...
								function revertComments() {	
									...
									var request = new HTTPRequest("POST", "<?php echo $blogURL;?>/plugin/DEFENSIO/revert/?t=C");
									request.onSuccess = function() {
										document.getElementById('list-form').submit();
									}
									request.onError = function () {
...
?>
						...
						<div>
							<ul class="tabs-box">
							<li class="selected"><a href="<?php echo $pluginMenuURL; ?>&t=C"><span>Comments</span></a></li>
							<li><a href="<?php echo $pluginMenuURL; ?>&t=T"><span>Trackbacks</span></a></li>
							</ul>
						</div>
...
<?php
	if ($isNameFiltered) {
?>
												<a id="nameFilter<?php echo $currentNumber;?>-<?php echo $i;?>" class="block-icon bullet" href="<?php echo $blogURL;?>/owner/setting/etc/filter/change/?value=<?php echo urlencode(escapeJSInAttribute($comment['name']));?>&amp;mode=name&amp;command=unblock&amp;id=<?php echo $filter->id;?>" onclick="changeState(this,'<?php echo escapeJSInAttribute($comment['name']);?>', '<?php echo $filter->id;?>', 'name'); return false;" title="<?php echo _t('이 이름은 차단되었습니다. 클릭하시면 차단을 해제합니다.');?>"><span class="text"><?php echo _t('[차단됨]');?></span></a>
<?php
	} else {
?>
												<a id="nameFilter<?php echo $currentNumber;?>-<?php echo $i;?>" class="unblock-icon bullet" href="<?php echo $blogURL;?>/owner/setting/etc/filter/change/?value=<?php echo urlencode(escapeJSInAttribute($comment['name']));?>&amp;mode=name&amp;command=block&amp;id=<?php echo $filter->id;?>" onclick="changeState(this,'<?php echo escapeJSInAttribute($comment['name']);?>', '<?php echo $filter->id;?>', 'name'); return false;" title="<?php echo _t('이 이름은 차단되지 않았습니다. 클릭하시면 차단합니다.');?>"><span class="text"><?php echo _t('[허용됨]');?></span></a>
<?php
	}
?>
												<!--<a href="<?php echo $pluginMenuURL;?>&t=<?php echo $comment_TYPE;?>&n=<?php echo urlencode(escapeJSInAttribute($comment['name']));?>" title="<?php echo _t('이 이름으로 등록된 댓글 목록을 보여줍니다.');?>"><?php echo htmlspecialchars($comment['name']);?></a>-->
...
?>
											...
											<td class="selection"><span><?php echo number_format($comment['spaminess'] * 100, 2 , '.','') ?>%</span></td>
											<td class="revert">
												<a class="revert-button button" href="<?php echo $blogURL;?>/plugin/DEFENSIO/revert/?id=<?php echo $comment['id'];?>&t=C" onclick="revertComment(<?php echo $comment['id'];?>); return false;" title="<?php echo _t('이 댓글을 복원합니다.');?>"><span class="text"><?php echo _t('복원');?></span></a>
											</td>
											<td class="delete">
												<a class="delete-button button" href="<?php echo $blogURL;?>/plugin/DEFENSIO/delete/?id=<?php echo $comment['id'];?>&t=C" onclick="deleteComment(<?php echo $comment['id'];?>); return false;" title="<?php echo _t('이 댓글을 삭제합니다.');?>"><span class="text"><?php echo _t('삭제');?></span></a>
											</td>
						  				</tr>
<?php
}
...
$paging['postfix'] = '&name=Defensio/DEFENSIO';
...
?>
						...
						-->
							<div class="button-box">
							<a class="button all-delete-button" href="<?php echo $blogURL;?>/plugin/DEFENSIO/quarantine/?t=C" onclick="deleteCommentAll(); return false;" title="<?php echo _t('모든 댓글을 한 번에 삭제합니다.');?>"><span class="text">Quarantine All</span></a>
							</div>
						</div>
